...
class SRWP_DB {
	...
	public function __construct() {
		...
		$this->file         = get_temp_dir() . DIRECTORY_SEPARATOR . 'srwp_db_backup.sql';		
	}
	...
	public static function get_sizes() {
		...
		if ( is_array( $tables ) && ! empty( $tables ) ) {
			...
			foreach ( $tables as $table ) {
				...
				$sizes[$table['Name']] = sprintf( __( '(%s MB)', 'search-replace-wp' ), $size );
			}
	...
	public function get_max_results() {
		$max_results = get_option( 'srwp_max_results' ) ? get_option( 'srwp_max_results' ) : 60;
	...
	public function get_page_size() {
		$page_size = get_option( 'srwp_page_size' ) ? get_option( 'srwp_page_size' ) : 20000;
	...
	public function rename_temp_tables() {
		...
		set_transient( 'srwp_results', $results );
	}
	...
	public function srdb( $table, $page, $args ) {
		...
		$args['search_for'] 	= str_replace( '#SRWP_BACKSLASH#', '\\', $args['search_for'] );
		$args['replace_with'] 	= str_replace( '#SRWP_BACKSLASH#', '\\', $args['replace_with'] );
		...
		foreach ( $data as $row ) {
			...
			foreach( $columns as $column ) {
				...
				if ( $this->wpdb->options === $table ) {
					...
					// Skip any SRWP options as they may contain the search field.
					...
					if ( isset( $update_later ) && true === $update_later ) {
						...
						if ( $edited_data != $data_to_fix ) {
							...
							update_option( 'srwp_update_site_url', $edited_data );
							continue;
						}
					}

					if ( '_transient_srwp_results' === $data_to_fix || 'srwp_profiles' === $data_to_fix || 'srwp_update_site_url' === $data_to_fix || 'srwp_data' === $data_to_fix ) {
						$should_skip = true;
					}
			...
			if ( $args['dry_run'] === 'on' ) {
				...
				if ( ! $result ) {
					$table_report['errors'][] = sprintf( __( 'Error updating row: %d.', 'search-replace-wp' ), $current_row );
				} else {
					$table_report['updates']++;
				}
	...
	public function maybe_update_site_url() {
		$option = get_option( 'srwp_update_site_url' );

		if ( $option ) {
			update_option( 'siteurl', $option );
			delete_option( 'srwp_update_site_url' );
			return true;
		}

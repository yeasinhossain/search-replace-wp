...
class SRWP_AJAX {
	...
	public function init() {
		...
		add_action( 'init', array( $this, 'do_srwp_ajax' ), 2 );
	...
	public static function get_endpoint() {
		return esc_url_raw( get_admin_url() . 'tools.php?page=search-replace-wp&srwp-ajax=' );
	}

	/**
	 * Set SRWP AJAX constant and headers.
	 * @access public
	 */
	public function define_ajax() {

		if ( isset( $_GET['srwp-ajax'] ) && ! empty( $_GET['srwp-ajax'] ) ) {
	 ...
	 * @access public
	 */
	public function do_srwp_ajax() {
		global $wp_query;

		if ( isset( $_GET['srwp-ajax'] ) && ! empty( $_GET['srwp-ajax'] ) ) {
			$wp_query->set( 'srwp-ajax', sanitize_text_field( $_GET['srwp-ajax'] ) );
		}

		if ( $action = $wp_query->get( 'srwp-ajax' ) ) {
			do_action( 'srwp_ajax_' . sanitize_text_field( $action ) );
			die();
		}
	}
	...
	public function add_ajax_actions() {
		...
		foreach ( $actions as $action ) {
			add_action( 'srwp_ajax_' . $action, array( $this, $action ) );
		}
	}
	...
	public function process_search_replace() {
		...
		if ( ! check_admin_referer( 'srwp_ajax_nonce', 'srwp_ajax_nonce' ) ) {
			return;
		}

		// Initialize the DB class.
		$db   = new SRWP_DB();
		$step = absint( $_REQUEST['srwp_step'] );
		$page = isset( $_REQUEST['srwp_page'] ) ? absint( $_REQUEST['srwp_page'] ) : 0;

		// Any operations that should only be performed at the beginning.
		if ( $step === 0 && $page === 0 ) {
			$args = array();
			parse_str( $_POST['srwp_data'], $args );
			...
			delete_transient( 'srwp_results' );
			delete_option( 'srwp_data' );
			...
			if ( '' !== $args['save_profile'] ) {
				...
				SRWP_Admin::save_profile( $profile );
			}
		} else {
			$args = get_option( 'srwp_data' );
		}
		...
		if ( isset( $args['select_tables'][$step] ) ) {
			...
			if ( isset( $args['select_tables'][$step] ) ) {
				...
				$message = sprintf(
					__( 'Processing table %d of %d: %s', 'search-replace-wp' ),
					$step + 1,
			...
			if ( $args['is_backup'] ) {
				$result = array(
					...
					'srwp_data' 		=> build_query( array( 'search_replace_complete' => 'true' ) )
				);
		...
		$result = array(
			...
			'url' 				=> get_admin_url() . 'tools.php?page=search-replace-wp&tab=srwp_search_replace&result=true',
			'srwp_data' 			=> build_query( $args )
		);
	...
	/**
	 * Helper function for assembling the SRWP Results.
	 * @access public
	...
	public function append_report( $table, $report, $args ) {
		...
		$results 		= get_transient( 'srwp_results' ) ? get_transient( 'srwp_results') : array();
		$num_results 	= get_option( 'srwp_max_results' ) ? absint( get_option( 'srwp_max_results' ) ) : 20;
		...
		if ( ! set_transient( 'srwp_results', $results, DAY_IN_SECONDS ) ) {
			return false;
		}
	...
	public function process_backup() {
		...
		$args = array();
		parse_str( $_REQUEST['srwp_data'], $args );

		// Initialize the DB class.
		$db 						= new SRWP_DB();
		$step 						= isset( $_REQUEST['srwp_step'] ) ? absint( $_REQUEST['srwp_step'] ) : 0;
		$page 						= isset( $_REQUEST['srwp_page'] ) ? absint( $_REQUEST['srwp_page'] ) : 0;
		...
		$completed_pages 			= isset( $args['completed_pages'] ) ? absint( $args['completed_pages'] ) : 0;
		$profile 					= isset( $args['srwp_backup_profile'] ) ? $args['srwp_backup_profile'] : false;
			...
			if ( $args['profile'] && ! $args['search_replace_complete'] ) {
				...
				if ( isset( $profiles[$profile] ) ) {
					...
					$result = array(
						...
						'message'		=> __( 'Starting search/replace...', 'search-replace-wp' ),
						'srwp_data'		=> build_query( $profiles[$profile] )
					);
		...
		if ( isset( $tables[$step] ) ) {
			...
			if ( isset( $tables[$step] ) ) {
				...
				$message = sprintf( __( 'Backing up table: "%s"...', 'search-replace-wp' ), $msg_tbl );
			}
			...
			if ( '' !== get_option( 'srwp_enable_gzip' ) ) {
				SRWP_Admin::compress_file( $db->file );
			}

		}

		$download_url = wp_nonce_url( get_admin_url() . 'admin-post.php?action=srwp_download_backup', 'srwp_download_backup', 'srwp_download_backup' );
	...
	public function process_import() {
		...
		if ( is_array( $_POST['srwp_data'] ) ) {
			$args = $_POST['srwp_data'];
		} else {
			$args = array();
			parse_str( $_POST['srwp_data'], $args );
		}
		...
		$result = array(
			...
			'page' 				=> $page,
			'percentage' 		=> $percentage,
			'url'				=> get_admin_url() . 'tools.php?page=search-replace-wp&import=true&result=true',
			'srwp_data'			=> $srwp_data
		);
		...
		if ( 'done' === $step ) {
			...
			if ( 'undefined' !== $profile && '' !== $profile ) {
				...
				if ( isset( $profiles[$profile] ) ) {
					...
					$result['message'] 		= __( 'Running search/replace...', 'search-replace-wp' );
					$result['srwp_data'] 	= build_query( $profiles[$profile] );
				} else {
	...
	public function upload_import() {
		...
		$manual_file = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'srwp_db_backup.sql';

		// Figure out which file we want to import.
		if ( isset( $_FILES['srwp_import_file' ] ) ) {

			$ext = substr( $_FILES['srwp_import_file']['name'], -3 );
			...
			if ( 'sql' === $ext || '.gz' === $ext ) {
				...
				$temp = $_FILES['srwp_import_file']['tmp_name'];
				$dest = $db->file;
				$upload_method 	= 'ajax';
				...
				if ( '.gz' === $ext ) {
					SRWP_Admin::decompress_file( $dest, $db->file );
				}
